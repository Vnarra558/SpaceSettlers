...
public class VenkatClient extends TeamClient {
    ...
    private static final String File_Name = "narr0004/ship_data.csv";
    ...
    private MyLogisticRegression model;
    /**
     * @param startingEnergy The starting energy of the ship.
     * @param pathDistance The distance of the path traveled by the ship.
     * @param isMoveable An integer indicating whether the ship is moveable (1) or not (0).
     * @param reachedGoal A boolean indicating whether the goal has been reached.
     */
    private void saveDataToFile(double startingEnergy, double pathDistance,int isMoveable, boolean reachedGoal) {

        try (BufferedWriter writer = new BufferedWriter(new FileWriter(File_Name, true))) {
            writer.write(String.format("%f,%f,%d,%d", startingEnergy, pathDistance,isMoveable, reachedGoal ? 1 : 0));
            writer.newLine();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//                System.out.println("Main Path size: " + path);
//                System.out.println("ship staring energy: "+ ship.getEnergy());

                // Here I'm collecting the data about  shipStartingEnergy , pathDistance, isMoveable and also if it reached or not.
                isMoveable = asteroid.isMoveable() ? 1 : 0;
                startingEnergy = ship.getEnergy();
                pathDistance = space.findShortestDistance(allVertices.get(0).getPosition(), allVertices.get(allVertices.size() - 1).getPosition());
                double[] features = new double[]{startingEnergy, pathDistance, isMoveable};
                int prediction = MyLogisticRegression.predict(features);
                dataSaved = false;
                if (prediction == 1) {
                    System.out.println("model predicted that the ship can collect the Asteroid");
...
//                    System.out.println("Prediction is 0");
                    currentTarget = null;
                    isGoalReached = true;
                }
            }
            // once path found, travel
                ...
                if( isGoalReached == false) {
                    ...
                    if (targetNotPresent(path.get(path.size() - 1).getPosition(),space)) {
                        System.out.println("Target is moved from the previous available location");
//                        saveDataToFile(startingEnergy, pathDistance,isMoveable, isGoalReached);
                        currentTarget = null;
                        nextVertex = 1;
                    }
...
//                    saveDataToFile(startingEnergy, pathDistance, isMoveable, true);
                    currentTarget = null;
                    isGoalReached = true;
                }
